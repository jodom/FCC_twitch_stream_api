// defaults
var default_logo = "assets/img/tv.png";
var offline_status = 'Offline';
// sample data reply from twicth API
var data = [
  {
    "stream": {
      "mature": false,
...
function listAllStreamers(){
  ...
  var html = $("#streamer");
	...
	data.forEach(streamer =>{
		if (streamer.hasOwnProperty("stream")){
      if (streamer.stream === null){
        streamers += "<div class='streamer'><div class='image'><img src='"+ default_logo +"' alt='TTV'></div><div class='details'><div class='name'><h3 id='display_name'>"+ streamer.display_name +"</h3></div><div class='playing'><a href='"+ streamer._links.channel +"' alt='channel link' target='_blank'><p id='current'>"+ offline_status +"</p></a></div></div></div>";
      }else{      
        streamers += "<div class='streamer'><div class='image'><img src='"+ streamer.stream.logo +"' alt='TTV'></div><div class='details'><div class='name'><h3 id='display_name'>"+ streamer.stream.display_name +"</h3></div><div class='playing'><a href='"+ streamer._links.channel +"' alt='channel link' target='_blank'><p id='current'>"+ streamer.stream.status +"</p></a></div></div></div>"; 
      }
    }
  });
  $("#content").html(streamers);
}

function listOnlineStreamers(){
  // filter for online accounts
  var online = data.filter( streamer => {return streamer.stream });
...
function listOfflineStreamers(){
  // filter for offline accounts
  var offline = data.filter( streamer => {return streamer.stream === null });

  var streamers = "";
  offline.forEach(streamer =>{
    streamers += "<div class='streamer'><div class='image'><img src='"+ default_logo +"' alt='TTV'></div><div class='details'><div class='name'><h3 id='display_name'>"+ streamer.display_name +"</h3></div><div class='playing'><a href='"+ streamer._links.channel +"' alt='channel link' target='_blank'><p id='current'>"+ offline_status +"</p></a></div></div></div>";
  });
  $("#content").html(streamers);
}
